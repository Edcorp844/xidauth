...
impl AuthService {
    ...
    pub fn register_user(&mut self, user_id: &str, user_info: &HashMap<&str, &str>) -> Result<(), RedisError> {
    ...
    pub fn authenticate_user(&mut self, user_id: &str, password: &str) -> Result<bool, RedisError> {
        // Retrieve user data from Redis as JSON
        let user_data: Option<String> = self.db.get_json_value(user_id)?;
        
        // Check if user data exists
        if let Some(data) = user_data {
            // Deserialize JSON data to access the password
            let user_info: HashMap<String, String> = serde_json::from_str(&data)?;
    ...
    pub fn store_oauth_token(&mut self, user_id: &str, token: &str, expiry_secs: usize) -> Result<(), RedisError> {
        self.db.set_value(&format!("oauth_token:{}", user_id), token)?;
        self.db.expire_key(&format!("oauth_token:{}", user_id), expiry_secs)?;
        Ok(())
    }
    ...
    pub fn set_session(&mut self, session_id: &str, user_id: &str, expiry_secs: usize) -> Result<(), RedisError> {
